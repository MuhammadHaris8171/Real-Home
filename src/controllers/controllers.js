...
const rows = await executeQuery(`
  (
    ...
    ORDER BY r_avg.avg_rating DESC, RAND()
    LIMIT 3
  )
  UNION
  (
    ...
    FROM products p
    WHERE NOT EXISTS (
      SELECT 1 FROM ratings r WHERE r.product_id = p.id
    )
    ORDER BY RAND()
    LIMIT 3
  );
...
// res.status(200).json(rows);
    ...
    // Optional: Slice result if you only want 3 products total
    // (UNION may return 3 + 3 = 6 max)
